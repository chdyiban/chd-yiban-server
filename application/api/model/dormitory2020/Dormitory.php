...
class Dormitory extends Model
{
    ...
    //选宿结果发出的模板消息id
    const TEMPLATE_ID = "X8oR07QAW1cA7h4-loY1uQlTAr_IMcSpD8OY_s-hATI";
    //模板消息跳转url
    const TEMPLATE_URL = "";
    /**
     * 填写家庭问卷信息
     ...
     * @param array $userinfo
     */
    public function sendMsg($userInfo)
    {
        if(empty($userInfo["step"]["step"]) || empty($userInfo["unionid"]) ){
            return false;
        }
        $step = $userInfo["step"]["step"];
        $XH = $userInfo["XH"];
        $checkBindInfo = Db::name("wx_unionid_user")->where("unionid",$userInfo["unionid"])->find();
        $dormitoryResult = Db::name("fresh_result")->where("XH",$XH)->find();
        if (!empty($checkBindInfo) && !empty($dormitoryResult) &&$checkBindInfo["portal_id"] == $XH  ) {
            $url = self::TEMPLATE_URL;
            $open_id = $checkBindInfo['open_id'];
            $template_id = self::TEMPLATE_ID;
            $ssResult = $dormitoryResult["SSDM"]."-".$dormitoryResult["CH"];
            $data = [
                'first' => [
                    'value' => "您好，您的宿舍选取结果如下:",
                    'color' => '#000066',
            ...
            if ($step=="SML") {
                $data["remark"]["value"] = "当前选宿阶段为模拟选宿，选宿结果到期清除，与正式选宿无关。";
            } elseif ($step == "FML") {
                $data["remark"]["value"] = "当前选宿阶段为正式选宿，选宿结果作为入学宿舍，有任何问题请联系客服人员。";
            } 
